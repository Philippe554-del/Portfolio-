...
/* ================================================================
   4. RATE LIMITING
================================================================ */
app.use(rateLimit({ windowMs: 60000, max: 120, standardHeaders: true, legacyHeaders: false, message: { error: 'Trop de requêtes.' } }));
const contactLimiter = rateLimit({ windowMs: 600000, max: 5, standardHeaders: true, legacyHeaders: false, message: { error: 'Trop de tentatives. Réessayez dans 10 minutes.' } });
const loginLimiter   = rateLimit({ windowMs: 900000, max: 10, skipSuccessfulRequests: true, standardHeaders: true, legacyHeaders: false, message: { error: 'Trop de tentatives. Réessayez dans 15 minutes.' } });
const adminLimiter   = rateLimit({ windowMs: 900000, max: 200, standardHeaders: true, legacyHeaders: false });
...
async function auth(req, res, next) {
  ...
  let payload;
  try {
    payload = jwt.verify(token, process.env.JWT_SECRET, { issuer: 'portfolio-admin', audience: 'portfolio-admin' });
  } catch (err) {
...
app.post('/api/admin/login', loginLimiter, async (req, res) => {
  try {
    ...
    const token = jwt.sign({ id: rows[0].id, email: rows[0].email, jti }, process.env.JWT_SECRET,
  ...
  try {
    ...
    await transporter.sendMail({
      ...
      html:    `<div style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto;padding:20px">
                  ...
                  <p style="color:#888;font-size:12px">Philippe Hountondji — ${process.env.GMAIL_USER}</p>
                </div>`
    });
...
// ✅ CORRECTION CV : route explicite AVANT les middlewares statiques généraux
// Sert frontend/cv/ à l'URL /cv/
// Content-Disposition: inline → s'affiche dans le navigateur (pas de téléchargement forcé)
...
app.use(function (err, req, res, next) {
  ...
  res.status(500).json({ error: process.env.NODE_ENV === 'production' ? 'Erreur interne du serveur.' : err.message });
});

/* ================================================================
   12. DÉMARRAGE + ARRÊT PROPRE
================================================================ */
connectDB()
  .then(function () {
