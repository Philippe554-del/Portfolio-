...
// ✅ CV : route GET explicite — garantit les bons headers PDF
// DOIT être avant express.static(frontend) sinon frontend intercepte d'abord
app.get('/cv/:filename', function (req, res) {
  ...
  res.sendFile(filePath, function (err) {
    if (err) {
      console.error('[CV] Fichier introuvable :', filePath);
      res.status(404).json({ error: 'CV introuvable. Vérifiez que le fichier existe dans frontend/cv/' });
    }
  });
});
