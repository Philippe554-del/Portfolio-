(function () {
  ...
  // chart.js chargé en fallback si pas encore dispo
  ...
  // gestion du token (session ou local selon "se souvenir")
  function getToken() {
  ...
  // wrapper fetch avec auth header
  ...
  function showDashboard() {
    ...
    // petit délai pour laisser le DOM se mettre en place
    setTimeout(function () {
      loadOverviewData();
      updateMessageCount();
    }, 100);
  ...
  // bouton retour portfolio injecté dans le footer de la sidebar
  function createBackToPortfolioButton() {
    ...
    backButton.addEventListener('click', function () { window.location.href = PORTFOLIO_URL; });
  ...
  document.querySelectorAll('.nav-item[data-page]').forEach(function (item) {
    ...
    // gestion touch séparée pour éviter le double-clic sur mobile
  ...
  function openSidebar()  { if (sidebar) sidebar.classList.add('active'); sidebarOverlay.style.display = 'block'; }
  function closeSidebar() { if (sidebar) sidebar.classList.remove('active'); sidebarOverlay.style.display = 'none'; }

  // debounce sur le toggle pour éviter les doubles clics rapides
  var _lastToggle = 0;
  ...
  sidebarOverlay.addEventListener('touchend', function (e) { e.preventDefault(); closeSidebar(); }, { passive: false });
  ...
  if (notifBtn) {
    notifBtn.addEventListener('click', function () {
      apiRequest('GET', '/api/admin/stats').then(function (res) {
        ...
        showNotification(unread === 0 ? 'Aucune nouvelle notification' : 'Vous avez ' + unread + ' message(s) non lu(s)', 'info');
      }).catch(function () { showNotification('Erreur lors du chargement', 'error'); });
    });
  }
  ...
  function loadOverviewData() {
    ...
    apiRequest('GET', '/api/admin/stats')
      .then(function (res) {
        if (res.status !== 200) { showNotification('Erreur chargement stats: ' + (res.data.error || res.status), 'error'); return; }
      ...
      .catch(function (err) { showNotification('Erreur réseau: ' + err.message, 'error'); });
  }
  ...
  // page messages avec recherche + filtre + tri
  ...
  // ouverture d'un message dans la modal
  ...
  // helper pour les champs de la modal
  ...
  function toggleRead(id) {
    apiRequest('PATCH', '/api/admin/messages/' + id + '/read')
      .then(function () { showNotification('Statut mis à jour', 'success'); loadMessages(); loadOverviewData(); })
      .catch(console.error);
  }
  ...
  function loadAnalytics() {
    if (!getToken()) { showNotification('Session expirée, reconnectez-vous', 'error'); return; }
    apiRequest('GET', '/api/admin/stats')
      .then(function (res) {
    ...
    if (ctx2) {
      ...
      try {
        statusChart = new Chart(ctx2, {
          ...
          data: {
            labels: ['Lus', 'Non lus'],
            datasets: [{ data: [readCount, unreadCount], backgroundColor: ['#10B981', '#FF6B35'], borderWidth: 0 }]
          },
          options: {
            responsive: true,
            maintainAspectRatio: false,
  ...
  // date relative (à l'instant, il y a X min, etc.)
  ...
  // mise à jour du badge messages non lus dans la nav
  ...
  // styles des notifications (injectés une seule fois)
  ...
  // modal de confirmation custom (remplace les confirm() natifs)
