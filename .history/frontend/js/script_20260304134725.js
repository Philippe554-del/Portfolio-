(function () {
  ...
  document.addEventListener('DOMContentLoaded', function () {
    ...
    function openMenu() {
      ...
      // ✅ PAS de overflow:hidden sur body — ça bloque window.scrollTo après
      // L'overlay empêche visuellement le scroll sans bloquer le DOM
    }
    ...
    function closeMenu() {
      ...
      // ✅ Rien à réinitialiser puisqu'on ne bloque plus le scroll
    }

    overlay.addEventListener('click', closeMenu);

    // ✅ CORRECTION MOBILE : on utilise UNIQUEMENT touchend + click
    // touchstart + click = double déclenchement sur mobile (menu s'ouvre ET se referme)
    // La variable _touchHandled empêche le click de se déclencher après un touch
    ...
    if (hamburger) {
      hamburger.addEventListener('touchend', function (e) {
        _touchHandled = true;
        toggleMenu(e);
        // Réinitialise après 500ms (laisse le temps au click fantôme de passer)
        setTimeout(function () { _touchHandled = false; }, 500);
      }, { passive: false });

      hamburger.addEventListener('click', function (e) {
        if (_touchHandled) return; // Ignore le click fantôme après touch
        toggleMenu(e);
      });
    }
    ...
    /* ================================================================
       SCROLL VERS SECTION — VERSION DÉFINITIVE
       
       PROBLÈME IDENTIFIÉ : sur mobile, après closeMenu(), le body
       retrouve overflow:auto ET scroll-behavior peut interférer.
       On force scrollTo avec un délai suffisant pour que le navigateur
       ait bien relâché le blocage du scroll.
       
       SOLUTION : on mémorise l'ID cible AVANT de fermer le menu,
       puis on attend 400ms (durée animation menu) avant de scroller.
       On utilise window.scrollTo avec comportement smooth, et un
       fallback immédiat si smooth n'est pas supporté.
    ================================================================ */
    ...
    // ✅ LIENS DU MENU MOBILE — gestion touch ET click sans conflit
    function bindMenuLinks() {
      ...
      navLinks.querySelectorAll('a[href^="#"]').forEach(function (link) {
        ...
        function handleNavLink(e) {
          ...
          if (isMenuOpen) {
            closeMenu();
            // ✅ Délai court — plus besoin d'attendre overflow car on ne le bloque plus
            setTimeout(function () {
        ...
        // ✅ touchend pour mobile (réactif, pas de délai 300ms)
        ...
        // ✅ click pour desktop — ignoré si déjà géré par touch
    ...
    document.addEventListener('click', function (e) {
      ...
      if (!link) return;
      // Exclut les liens du menu (déjà gérés ci-dessus)
    ...
    /* ================================================================
       CV — BOUTON "CONSULTER EN LIGNE"
       ✅ Ouvre le PDF directement via la route /cv/ du serveur
       ✅ Vérifie que l'URL ne redirige pas vers un domaine externe
    ================================================================ */
    ...
    if (cvViewBtn) {
      cvViewBtn.addEventListener('click', function (e) {
        ...
        // Anti open-redirect : refuse toute URL pointant vers un autre domaine
        if (/^https?:\/\//i.test(href) && href.indexOf(window.location.origin) !== 0) return;
        // S'assure que l'URL commence par / (relative au serveur actuel)
