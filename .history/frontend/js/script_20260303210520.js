(function () {
  ...
  document.addEventListener('DOMContentLoaded', function () {
    ...
    /* ===== Anti double-déclenchement touch + click ===== */
    ...
    /* =====================================================
       CORRECTION SCROLL : clic sur lien du menu → section
       On cible TOUS les liens #anchor du site (menu + footer)
    ===================================================== */
    ...
    document.querySelectorAll('a[href^="#"]').forEach(function (link) {
      link.addEventListener('click', function (e) {
        ...
        if (!targetId) return; // lien "#" vide → ignore

        var target = document.getElementById(targetId);
        if (!target) return; // pas une ancre interne → comportement normal

        e.preventDefault();

        var isMenuLink = link.closest('.nav-links');
        if (isMenuLink) {
          // Ferme le menu, puis scrolle après l'animation
          closeMenu();
          setTimeout(function () { scrollToSection(targetId); }, 320);
        } else {
          // Lien hors menu (footer, boutons hero, etc.) → scroll direct
    ...
    /* =====================================================
       CORRECTION CV — Télécharger & Consulter en ligne
       Le PDF est un fichier statique dans /cv/
       On contourne le serveur Node.js en construisant
       l'URL absolue basée sur l'origine de la page
    ===================================================== */
    var CV_PATH = 'cv/cv_philippe_hountondji.pdf';

    // Chemin absolu depuis la racine du site GitHub Pages
    // ex: https://hountondji-philippe.github.io/portfolio/cv/cv_philippe_hountondji.pdf
    function getCvUrl() {
      var base = window.location.href.split('#')[0];
      // Supprime index.html si présent
      base = base.replace(/index\.html$/, '');
      // S'assure que la base se termine par /
    ...
    if (cvDownloadBtn) {
      cvDownloadBtn.addEventListener('click', function (e) {
        ...
        // Force l'attribut href correct au moment du clic
        cvDownloadBtn.setAttribute('href', getCvUrl());
        cvDownloadBtn.setAttribute('download', 'CV_Philippe_Hountondji.pdf');
        // Laisse le comportement natif download se produire
      });
    }
    ...
    if (cvViewBtn) {
      cvViewBtn.addEventListener('click', function (e) {
        e.preventDefault();
        // Ouvre le PDF dans un nouvel onglet — contourne le routeur Node.js
        window.open(getCvUrl(), '_blank', 'noopener,noreferrer');
      });
    }
