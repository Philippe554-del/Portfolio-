(function () {
  ...
  function checkAuth() {
    ...
    // ✅ Valide le token côté serveur AVANT d'afficher le dashboard
    // Évite l'affichage d'un dashboard qui charge indéfiniment avec un token invalide
    ...
    .then(function (r) {
      if (r.status === 401 || r.status === 403) {
        // Token invalide ou expiré → forcer reconnexion
        clearToken();
        showLoginPage();
      } else {
        showDashboard();
    ...
    .catch(function () {
      // Erreur réseau → on tente quand même d'afficher le dashboard
      showDashboard();
    });
  }
