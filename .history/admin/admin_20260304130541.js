(function () {
  ...
  function showAnalyticsError(msg) {
    var containers = ['messagesChart', 'statusChart'];
    containers.forEach(function (id) {
      var el = document.getElementById(id);
      if (el) {
        var parent = el.parentElement;
        if (parent) parent.innerHTML = '<p style="color:#EF4444;text-align:center;padding:2rem;font-size:0.9rem"><i class="fas fa-exclamation-triangle"></i> ' + msg + '</p>';
      }
    });
  }
  ...
  function renderCharts(s) {
    ...
    if (ctx2) {
      ...
      // Si les deux valeurs sont 0 → affiche message vide
      if (readCount === 0 && unreadCount === 0) {
        ctx2.parentElement.innerHTML = '<p style="color:#B4B8D4;text-align:center;padding:2rem;font-size:0.9rem"><i class="fas fa-inbox"></i><br>Aucun message reçu</p>';
      } else {
        try {
