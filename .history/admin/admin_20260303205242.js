(function () {
  ...
  /* =====================================================
     AUTH — CORRECTION PRINCIPALE
     Le dashboard est caché via display:none dès le HTML
     (class="hidden"). On contrôle tout ici proprement.
  ===================================================== */
  function checkAuth() {
    if (!getToken()) {
      showLoginPage();
  ...
  function showLoginPage() {
    ...
    /* Cache le dashboard COMPLÈTEMENT */
    if (dashboard) {
      dashboard.style.display = 'none';
      dashboard.classList.add('hidden');
    }

    /* Affiche le login PAR-DESSUS tout */
    if (loginPage) {
      ...
      loginPage.style.background   = 'var(--bg-primary, #0A0E27)';
    }

    /* Détruire les graphiques si on revient au login */
  ...
  if (menuToggle) {
    function toggleSidebar(e) {
      ...
      e.stopPropagation();
      var now = Date.now();
      if (now - _sidebarLastToggle < 400) return;
  ...
  function loadOverviewData() {
    apiRequest('GET', '/api/admin/stats')
      .then(function (res) {
        ...
        setText('messageCount',   s.unread > 0 ? s.unread : '');
        loadRecentMessages();
      })
      .catch(console.error);
  }
          ...
          body.innerHTML =
            ...
            '<div id="replySection" style="margin-top:1.5rem;border-top:1px solid rgba(255,255,255,0.1);padding-top:1.5rem">' +
              ...
              '<div style="margin-bottom:1rem">' +
                ...
                '<textarea id="replyText" rows="7" style="width:100%;padding:0.75rem;background:rgba(255,255,255,0.05);border:1px solid rgba(255,255,255,0.1);border-radius:8px;color:#fff;font-size:0.9rem;resize:vertical;font-family:inherit;box-sizing:border-box;outline:none;line-height:1.6" onfocus="this.style.borderColor=\'#FF6B35\'" onblur="this.style.borderColor=\'rgba(255,255,255,0.1)\'">Bonjour ' + esc(msg.name) + ',\n\n\n\nCordialement,\nPhilippe Hountondji\nhountondjiphilippe58@gmail.com\n+229 01 58 15 69 30</textarea>' +
              '</div>' +
    ...
    apiRequest('POST', '/api/admin/send-reply', { to: to, subject: subject, message: message, messageId: msg.id })
      .then(function (res) {
        if (res.status === 200 && res.data.success) {
          status.innerHTML = '<span style="color:#10B981"><i class="fas fa-check-circle"></i> Email envoyé avec succès à ' + esc(to) + ' !</span>';
  ...
  function renderCharts(s) {
    ...
    if (labels.length === 0) { labels = ["Aujourd'hui"]; counts = [parseInt(s.total || 0)]; }
    ...
    if (ctx2) {
      ...
      try {
        statusChart = new Chart(ctx2, {
          ...
          options: {
            responsive: true, maintainAspectRatio: false,
            plugins: { legend: { labels: { color: '#B4B8D4', font: { size: 13 }, padding: 20 } } },
            cutout: '65%'
          }
        });
